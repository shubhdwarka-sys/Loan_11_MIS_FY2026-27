...
function clearEmpFields_(ss, sheet, row) {
  ...
  var accSheet = ss.getSheetByName(TCO.SHEET.ACC);
  var rtoSheet = ss.getSheetByName(TCO.SHEET.RTO);
  if (accSheet) accSheet.getRange(row, TCO.ACC.EXEC).setValue('').setFontStyle('normal');
  if (rtoSheet) {
    rtoSheet.getRange(row, TCO.RTO.EXEC).setValue('').setFontStyle('normal');
    rtoSheet.getRange(row, TCO.RTO.BRANCH).setValue('').setFontStyle('normal');
  }
}
...
function updateMasterData_(ss) {
  try {
    ...
    var dmData  = dm.getDataRange().getValues();
    var accData = acc ? acc.getDataRange().getValues() : [];
    var rtoData = rto ? rto.getDataRange().getValues() : [];

    // ── Build DM NO → row maps for ACC + RTO ──
    var accMap = {}, rtoMap = {};
    for (var a = TCO.DATA_ROW - 1; a < accData.length; a++) {
      var ak = String(accData[a][TCO.ACC.DM_NO - 1]).trim();
      if (ak) accMap[ak] = accData[a];
    }
    for (var r = TCO.DATA_ROW - 1; r < rtoData.length; r++) {
      var rk = String(rtoData[r][TCO.RTO.DM_NO - 1]).trim();
      if (rk) rtoMap[rk] = rtoData[r];
    }
